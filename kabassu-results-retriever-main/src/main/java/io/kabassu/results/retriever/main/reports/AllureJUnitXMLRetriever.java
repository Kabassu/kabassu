...
public class AllureJUnitXMLRetriever extends ReportsRetriever {
  ...
  public String retrieveReport() throws IOException, InterruptedException {
    ...
    generateReport(directories[0], directories[1].getName());
    return path;
  }
  ...
  private void generateReport(File directory, String outputDir)
    throws IOException, InterruptedException {
    ProcessBuilder processBuilder = new ProcessBuilder();
    processBuilder.directory(directory.getParentFile());
    if (SystemUtils.IS_OS_WINDOWS) {
      processBuilder.command(CommandLines.CMD, "/c", "allure generate "+directory.getName()+" --clean -o " + outputDir);
    } else {
      processBuilder.command(CommandLines.BASH, "-c", "allure generate "+directory.getName()+" --clean -o " + outputDir);
    }
